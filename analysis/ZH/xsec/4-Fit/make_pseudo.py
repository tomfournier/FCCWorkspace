...
hist_pseudo = make_pseudodata(inputDir, procs, procs_cfg, hName, arg.target, z_decays, h_decays, ecm=ecm, variation=arg.pert)
...
if arg.run:
    cmd = f"python3 4-Fit/fit.py --bias --target {arg.target} --pert {arg.pert}"
    os.system(cmd)
else:
    print('\n\n------------------------------------\n')
